...
public class MainProjectileUtil
{
    ...
    {
        var tempA = norm.cross(new Vec3(1, 0, 0));
        if(tempA.length() <= 0.0001)
            tempA = norm.cross(new Vec3(0, 1, 0));

        if(tempA.length() <= 0.0001)
            tempA = norm.cross(new Vec3(0, 0, 1));

        tempA = tempA.normalize();

        var a = tempA;
        var b = norm.cross(tempA).normalize();
